...
class GoogleMapsScraper:
    ...
    def _scrape_category(self, page, category, location, max_results, yield_callback=None):
        ...
        links_locator = page.locator('div[role="feed"] > div > div > a')
    ...
    def _scroll_feed(self, page, target_count):
        ...
        while True:
            links = feed_locator.locator('> div > div > a')
            count = links.count()
            ...
            if count == last_count:
                retries += 1
                if retries > 3:  # End of list reached or network slow
                    break
            else:
                retries = 0
            ...
            try:
                ...
                feed_locator.evaluate("element => element.scrollBy(0, 800)")
